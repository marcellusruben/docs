...
The default values for each setting are intelligently generated by the Planner based on your prompt description. However, in most cases, you’ll want to customize them to better fit your specific needs and use case. You can refer to this section of the xpander docs to learn more about these settings.
...
The Chat and Visual Tester provide an intuitive visual breakdown of how the agent interprets your prompt. You can track the agent’s workflow, including the input and output of each operation it uses to complete a task. Based on what you see, you can easily make adjustments, like adding or removing operations, which we’ll cover in the next section. This process significantly reduces the engineering effort needed to build and fine-tune an AI agent.
